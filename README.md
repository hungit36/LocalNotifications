...
- Send **Push Notifications** using add-on plugins, as [local_notifications_fcm]()
...
*Working progress percentages of local notifications plugin*
...
`local_notifications` plugin is incompatible with `flutter_local_notifications`, as both plugins will compete each other to accquire global notification resources to send notifications and to receive notification actions.

So, you **MUST not use** `flutter_local_notifications` with `local_notifications`. Local Notifications contains all features available in `flutter_local_notifications` plugin and more, so you can replace totally `flutter_local_notifications` in your project.
...
To use FCM services with Local Notifications, you need use the [Local Notifications FCM add-on plugin](https://pub.dev/packages/local_notifications_fcm).

Only using [local_notifications_fcm](https://pub.dev/packages/local_notifications_fcm) you will be capable to achieve all Firebase Cloud Messaging features + all Local Notifications features. To keep using firebase_messaging, you gonna need to do workarounds with silent push notifications, and this is disrecommended to display visual notifications and will result in several background penalities to your application.
    
So, you **MUST not use** `firebase_messaging` with `local_notifications`. Use `local_notifications` with `local_notifications_fcm` instead.
...
- Include Web support
- Finish the add-on plugin to enable Firebase Cloud Message with all the local features available. (accomplished)
...
- [Local Notifications for Flutter](#local-notifications-for-flutter)
    - [Features](#features)
...
- [🔷 Local Notification's Flowchart](#-local-notifications-flowchart)
...
* The support for `firebase_messaging` plugin is now deprecated, but all other firebase plugins still being supported. You need use the [Local's FCM add-on plugin](https://pub.dev/packages/local_notifications_fcm) to achieve all firebase messaging features, without violate the platform rules. This is the only way to fully integrated with local notifications, running all in native level.
...
Bellow are the obligatory configurations that your app must meet to use local_notifications:
...
To use Local Notifications and build your app correctly, you need to ensure to set some `build settings` options for your app targets. In your project view, click on *Runner -> Target Runner -> Build settings*...  
...
1. Add *local_notifications* as a dependency in your `pubspec.yaml` file.

```yaml
local_notifications: any # Any attribute updates automatically your source to the last version
```
...
class MyApp extends StatefulWidget {
  ...
  static const String name = 'Local Notifications - Example App';
...
class _MyAppState extends State<MyApp> {
  ...
  void initState() {
    ...
    LocalNotifications().setListeners(
...
```dart
LocalNotifications().isNotificationAllowed().then((isAllowed) {
  if (!isAllowed) {
    // This is just a basic example. For real apps, you must show some
    // friendly dialog box before call the request method.
    // This is very important to not harm the user experience
    LocalNotifications().requestPermissionToSendNotifications();
  }
});
```
...
1 . You must initialize all Local Notifications plugins, even if your app does not have permissions to send notifications.
...
```dart
void main() async {
    ReceivedAction? receivedAction = await LocalNotifications().getInitialNotificationAction(
...
With the examples bellow, you can check all the features and how to use the Local Notifications in pratice. The Simple Example app contains the basic structure to use Local Notifications, and the Complete Example App contains all Local Notification features to test.
...
2. Copy the example content at https://pub.dev/packages/local_notifications/example
...
# 🔷 Local Notification's Flowchart
...
![Local Notification's flowchart](https://user-images.githubusercontent.com/40064496/197368144-7bfcee7e-644a-4bdc-80f1-b4d38c2eaaff.png)
...
The local notifications event methods available to track your notifications are:
...
The notification action type defines how local notifications should handle the user actions.
...
```Dart
  static Future<List<NotificationPermission>> requestUserPermissions(
    ...
    List<NotificationPermission> permissionsAllowed = await LocalNotifications().checkPermissionList(
    ...
    List<NotificationPermission> lockedPermissions = await LocalNotifications().shouldShowRationaleToRequest(
        channelKey: channelKey,
    ...
    if(lockedPermissions.isEmpty){
      ...
      permissionsAllowed = await LocalNotifications().checkPermissionList(
          channelKey: channelKey,
          permissions: permissionsNeeded
      );
    }
      ...
      await showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            title: Text('Local Notifications needs your permission',
...
```Dart
  String localTimeZone = await LocalNotifications().getLocalTimeZoneIdentifier();
  String utcTimeZone = await LocalNotifications().getLocalTimeZoneIdentifier();
...
await LocalNotifications().createNotification(
  ...
  schedule: NotificationInterval(
      ...
      preciseAlarm: true,
      timezone: await LocalNotifications().getLocalTimeZoneIdentifier()
  );
```
...
```dart
  await LocalNotifications().createNotification(
      content: NotificationContent(
          id: id,
          channelKey: 'basic_channel',
          title: 'Emojis are local too! '+ Emojis.smille_face_with_tongue + Emojis.smille_rolling_on_the_floor_laughing + Emojis.emotion_red_heart,
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
   ...
   <application
        ...
        android:label="Local Notifications for Flutter">
        <activity
            android:name=".MainActivity"
...
On iOS there is no native channels, but local will handle your notification channels by the same way than Android. This way your app doesn't need to do workarounds to get the closest possible results in both platforms. You gonna write once and run anywhere.

Also, its possible to organize visualy the channel's in you Android's app channel page using `NotificationChannelGroup` in the `LocalNotifications().initialize` method and the property `channelGroupKey` in the respective channels. The channel group name can be updated at any time, but an channel only can be defined into a group when is created.

The main methods to manipulate notification channels are:

* LocalNotifications().setChannel: Create or update a notification channel.
* LocalNotifications().removeChannel: Remove a notification channel, closing all current notifications on that same channel.
...
**Issue:** local_notifications is not displaying images, playing custom sounds or showing icons on release mode
...
**Fix:** Please, remove the old target extensions and update your local_notification plugin to the last version available
...
```dart
import 'package:local_notifications/android_foreground_service.dart';
```
...
```xml
 <service android:name="me.carda.local_notifications.core.services.ForegroundService"
