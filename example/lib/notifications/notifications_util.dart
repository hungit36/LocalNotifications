...
import 'package:local_push_notifications/android_foreground_service.dart';
...
import 'package:local_push_notifications/local_notifications.dart'
    hide LocalDateUtils;
import 'package:local_push_notifications/local_notifications.dart' as utils
    show LocalDateUtils;

import 'package:local_push_notifications_example/models/media_model.dart';
import 'package:local_push_notifications_example/utils/common_functions.dart'
    if (dart.library.html) 'package:local_push_notifications_example/utils/common_web_functions.dart';
import 'package:local_push_notifications_example/utils/media_player_central.dart';
...
class NotificationUtils {
  static Future<bool> redirectToPermissionsPage() async {
    await LocalNotifications().showNotificationConfigPage();
    return await LocalNotifications().isNotificationAllowed();
  }
  ...
  static Future<void> redirectToAlarmPage() async {
    await LocalNotifications().showAlarmPage();
  }
  ...
  static Future<void> redirectToOverrideDndsPage() async {
    await LocalNotifications().showGlobalDndOverridePage();
  }
          ...
          builder: (context) => AlertDialog(
                ...
                content: Column(
                  ...
                  children: [
                    ...
                    const Text(
                      'Allow Local Notifications to send you beautiful notifications!',
                      maxLines: 4,
                      textAlign: TextAlign.center,
                    ),
                  ],
                ...
                actions: [
                  ...
                  TextButton(
                    onPressed: () async {
                      isAllowed = await LocalNotifications()
  ...
  static Future<List<NotificationPermission>> requestUserPermissions(
    ...
    List<NotificationPermission> permissionsAllowed =
        await LocalNotifications().checkPermissionList(
    ...
    List<NotificationPermission> lockedPermissions =
        await LocalNotifications().shouldShowRationaleToRequest(
    ...
    if (lockedPermissions.isEmpty) {
      ...
      await showDialog(
          ...
          builder: (context) => AlertDialog(
                ...
                title: Text(
                  'Local Notificaitons needs your permission',
                  textAlign: TextAlign.center,
                  maxLines: 2,
                ...
                actions: [
                  ...
                  TextButton(
                    onPressed: () async {
                      ...
                      // After the user come back, check if the permissions has successfully enabled
                      permissionsAllowed = await LocalNotifications()
                          .checkPermissionList(
                              channelKey: channelKey,
                              permissions: lockedPermissions);
  ...
  ************************************************ */
  ...
  static Future<void> showBasicNotification(int id) async {
    try {
      bool success = await LocalNotifications().createNotification(
  ...
  static Future<void> showNotificationFromJson(
    ...
    await LocalNotifications().createNotificationFromJsonData(jsonData);
  }
  ...
  ************************************************ */
  ...
  static Future<void> showBadgeNotification(int id, {int? badgeAmount}) async {
    await LocalNotifications().createNotification(
        ...
        schedule: NotificationInterval(
            interval: 5,
            timeZone:
                await LocalNotifications().getLocalTimeZoneIdentifier()));
  }
  ...
  static Future<int> getBadgeIndicator() async {
    int amount = await LocalNotifications().getGlobalBadgeCounter();
    return amount;
  }

  static Future<void> setBadgeIndicator(int amount) async {
    await LocalNotifications().setGlobalBadgeCounter(amount);
  }

  static Future<int> incrementBadgeIndicator() async {
    return await LocalNotifications().incrementGlobalBadgeCounter();
  }

  static Future<int> decrementBadgeIndicator() async {
    return await LocalNotifications().decrementGlobalBadgeCounter();
  }

  static Future<void> resetBadgeIndicator() async {
    await LocalNotifications().resetGlobalBadge();
  }
  ...
  ************************************************ */
  ...
  static Future<void> showLockedNotification(int id) async {
    LocalNotifications().setChannel(NotificationChannel(
  ...
  ************************************************ */
  ...
  static Future<void> showNotificationImportance(
    ...
    await LocalNotifications().setChannel(NotificationChannel(
        channelKey: channelKey,
        channelName: title,
  ...
  ************************************************ */
  ...
  static Future<void> createMessagingNotification(
    ...
    await LocalNotifications().createNotification(
        content: NotificationContent(
            id: createUniqueID(LocalNotifications.maxID),
            groupKey: groupKey,
            channelKey: channelKey,
            summary: chatName,
            title: username,
  ...
  ************************************************ */
  ...
  static Future<void> listScheduledNotifications(BuildContext context) async {
    List<NotificationModel> activeSchedules =
        await LocalNotifications().listScheduledNotifications();
  ...
  static Future<String> getCurrentTimeZone() {
    return LocalNotifications().getLocalTimeZoneIdentifier();
  }

  static Future<String> getUtcTimeZone() {
    return LocalNotifications().getUtcTimeZoneIdentifier();
  }
  ...
  static Future<void> showNotificationAtSchedulePreciseDate(
    ...
    String timeZoneIdentifier = LocalNotifications.localTimeZoneIdentifier;
    await LocalNotifications().createNotification(
        content: NotificationContent(
          id: -1,
          channelKey: 'scheduled',
          title: 'Just in time!',
          body:
              'This notification was schedule to shows at ${utils.LocalDateUtils.parseDateToString(scheduleTime.toLocal()) ?? '?'} $timeZoneIdentifier (${utils.LocalDateUtils.parseDateToString(scheduleTime.toUtc()) ?? '?'} utc)',
          notificationLayout: NotificationLayout.BigPicture,
  ...
  static Future<void> removeChannel() async {
    LocalNotifications().removeChannel('updated_channel');
  }

  static Future<void> dismissNotification(int id) async {
    await LocalNotifications().dismiss(id);
  }

  static Future<void> cancelSchedule(int id) async {
    await LocalNotifications().cancelSchedule(id);
  }
  ...
  static Future<void> dismissNotificationsByChannelKey(
    ...
    await LocalNotifications().dismissNotificationsByChannelKey(channelKey);
  }

  static Future<void> dismissNotificationsByGroupKey(String groupKey) async {
    await LocalNotifications().dismissNotificationsByGroupKey(groupKey);
  }

  static Future<void> cancelSchedulesByChannelKey(String channelKey) async {
    await LocalNotifications().cancelSchedulesByChannelKey(channelKey);
  }

  static Future<void> cancelSchedulesByGroupKey(String groupKey) async {
    await LocalNotifications().cancelSchedulesByGroupKey(groupKey);
  }

  static Future<void> cancelNotificationsByChannelKey(String channelKey) async {
    await LocalNotifications().cancelNotificationsByChannelKey(channelKey);
  }

  static Future<void> cancelNotificationsByGroupKey(String groupKey) async {
    await LocalNotifications().cancelNotificationsByGroupKey(groupKey);
  }

  static Future<void> dismissAllNotifications() async {
    await LocalNotifications().dismissAllNotifications();
  }

  static Future<void> cancelAllSchedules() async {
    await LocalNotifications().cancelAllSchedules();
  }
