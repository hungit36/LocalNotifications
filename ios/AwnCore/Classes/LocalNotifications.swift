...
public class LocalNotifications:
        NSObject,
        LocalActionEventListener,
        LocalNotificationEventListener,
        LocalLifeCycleEventListener,
        UIApplicationDelegate,
        UNUserNotificationCenterDelegate
{
    let TAG = "LocalNotifications"
    ...
    public static var localExtensions:LocalNotificationsExtension?
    ...
    public override init() {
        ...
        LocalNotifications.debug = isApplicationInDebug()
        ...
        if LocalNotifications.localExtensions == nil {
            throw ExceptionFactory
                    ...
                    .createNewLocalException(
                        ...
                        message: "Local's plugin extension reference was not found.",
                        detailedCode: ExceptionCode.DETAILED_INITIALIZATION_FAILED+".localNotifications.extensions")
        }
        
        LocalNotifications.localExtensions!.loadExternalExtensions()
        
        areDefaultsLoaded = true
    }
    
    private var isTheMainInstance = false
    public func attachAsMainInstance(usingLocalEventListener listener: LocalEventListener){
        ...
        subscribeOnLocalNotificationEvents(listener: listener)
        ...
        Logger.d(TAG, "Local notifications \(self.hash) attached to app instance");
    }
    
    public func detachAsMainInstance(listener: LocalEventListener){
        if !self.isTheMainInstance {
            return
        }
        ...
        unsubscribeOnLocalNotificationEvents(listener: listener)
        ...
        Logger.d(TAG, "Local notifications \(self.hash) detached from app instance");
    }
    ...
    public func initialize() {
        LocalNotifications.initialValues.removeAll()
        LocalNotifications.initialValues.merge(
    ...
    public func subscribeOnNotificationEvents(listener:LocalNotificationEventListener) {
        notificationEventListeners.append(listener)
    }
    
    public func unsubscribeOnNotificationEvents(listener:LocalNotificationEventListener) {
    ...
    private func notifyNotificationEvent(eventName:String, notificationReceived:NotificationReceived){
        notifyLocalEvent(eventType: eventName, content: notificationReceived.toMap())
    ...
    private lazy var notificationActionListeners = [LocalActionEventListener]()
    ...
    private func notifyActionEvent(fromEventNamed eventName:String, withActionReceived actionReceived:ActionReceived){
        notifyLocalEvent(eventType: eventName, content: actionReceived.toMap())
    ...
    private lazy var localEventListeners = [LocalEventListener]()
    
    public func subscribeOnLocalNotificationEvents(listener:LocalEventListener) {
        localEventListeners.append(listener)
    }
    
    public func unsubscribeOnLocalNotificationEvents(listener:LocalEventListener) {
        if let index = localEventListeners.firstIndex(where: {$0 === listener}) {
            localEventListeners.remove(at: index)
        }
    }
    
    private func notifyLocalEvent(eventType: String, content: [String : Any?]){
        for listener in localEventListeners {
            listener.onNewLocalEvent(eventType: eventType, content: content)
        }
    }
    ...
    public func initialize(
        ...
        LocalNotifications.debug = debug
        
        if(LocalNotifications.debug){
            Logger.d(TAG, "Local Notifications initialized")
        }
    }
    ...
    @objc public func didFinishLaunch(_ application: UIApplication) {
        ...
        if LocalNotifications.debug {
            Logger.d(TAG, "Local Notifications attached for iOS")
        }
    }
    ...
    public func userNotificationCenter(
        ...
        do {
            ...
            if !(error is LocalNotificationsException) {
                ExceptionFactory
                    .shared
