...
class DefaultsManager {
    ...
    public var extensionClassName:String? {
        get { return userDefaults.object(forKey: Definitions.LOCAL_EXTENSION_CLASS_NAME) as? String }
        set { userDefaults.setValue(newValue, forKey: Definitions.LOCAL_EXTENSION_CLASS_NAME) }
    }
    
    public var lastDisplayedDate:RealDateTime {
        get {
            let dateText:String? = userDefaults.object(forKey: Definitions.LOCAL_LAST_DISPLAYED_DATE) as? String
            
            Logger.d(DefaultsManager.TAG, "Local Notifications - UTC timezone : \(RealDateTime.utcTimeZone)")
            Logger.d(DefaultsManager.TAG, "Local Notifications - Local timezone : \(DateUtils.shared.localTimeZone)")
            
            guard let dateText:String = dateText else {
                return RealDateTime.init(fromTimeZone: RealDateTime.utcTimeZone)
            }
            
            Logger.d(DefaultsManager.TAG, "Local Notifications - last displayed date : \(dateText)")
        ...
        set { userDefaults.setValue(newValue.description, forKey: Definitions.LOCAL_LAST_DISPLAYED_DATE) }
    }
