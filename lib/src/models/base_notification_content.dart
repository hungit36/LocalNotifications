...
class BaseNotificationContent extends Model {
  ...
  BaseNotificationContent? fromMap(Map<String, dynamic> mapData) {
    ...
    _id = LocalAssertUtils.extractValue(NOTIFICATION_ID, mapData, int);
    _channelKey = LocalAssertUtils.extractValue(
        NOTIFICATION_CHANNEL_KEY, mapData, String);
    _groupKey = LocalAssertUtils.extractValue(
        NOTIFICATION_GROUP_KEY, mapData, String);

    _title =
        LocalAssertUtils.extractValue(NOTIFICATION_TITLE, mapData, String);
    _body = LocalAssertUtils.extractValue(NOTIFICATION_BODY, mapData, String);
    _summary =
        LocalAssertUtils.extractValue(NOTIFICATION_SUMMARY, mapData, String);
    _showWhen =
        LocalAssertUtils.extractValue(NOTIFICATION_SHOW_WHEN, mapData, bool);
    _icon = LocalAssertUtils.extractValue(NOTIFICATION_ICON, mapData, String);
    _largeIcon = LocalAssertUtils.extractValue(
        NOTIFICATION_LARGE_ICON, mapData, String);
    _bigPicture = LocalAssertUtils.extractValue(
        NOTIFICATION_BIG_PICTURE, mapData, String);
    _customSound = LocalAssertUtils.extractValue(
        NOTIFICATION_CUSTOM_SOUND, mapData, String);

    _wakeUpScreen = LocalAssertUtils.extractValue(
        NOTIFICATION_WAKE_UP_SCREEN, mapData, bool);
    _fullScreenIntent = LocalAssertUtils.extractValue(
        NOTIFICATION_FULL_SCREEN_INTENT, mapData, bool);
    _criticalAlert = LocalAssertUtils.extractValue(
    ...
    _roundedLargeIcon = LocalAssertUtils.extractValue(
        NOTIFICATION_ROUNDED_LARGE_ICON, mapData, bool);
    _roundedBigPicture = LocalAssertUtils.extractValue(
  ...
  ImageProvider? get bigPictureImage {
    ...
    return LocalBitmapUtils().getFromMediaPath(_bigPicture!);
  }

  ImageProvider? get largeIconImage {
    if (_largeIcon?.isEmpty ?? true) return null;
    return LocalBitmapUtils().getFromMediaPath(_largeIcon!);
  }
  ...
  String? get bodyWithoutHtml => LocalHtmlUtils.removeAllHtmlTags(_body);
